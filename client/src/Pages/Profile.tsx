        ...
        <Card>
          ...
          {isEditing ? (
            <div className="space-y-4">
              ...
              <Input label="Weight" type="number" value={formData.weight}
               onChange={(v)=>setFormData({...formData,weight:Number(v)})} 
               min={20} max={500} />
               
              <Input label="Height" type="number" value={formData.height}
               ...
               onChange={(v)=>setFormData({
                 ...formData,
                 goal: String(v) as 'lose' | 'gain' | 'maintain',
               })} options={goalOptions} 
             />
            ...
            <>
              <div className="space-y-4">
                {/* age */}
                <div className="flex items-start gap-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg transition-colors duration-200">
                  <div className="size-10 rounded-lg bg-blue-100 dark:bg-blue-900/20 flex items-center justify-center">
                    <Calendar className="size-5 text-blue-500 dark:text-slate-400" />
                  </div>
                  <div>
                ...
                {/* weight */}
                 ...
                 <div className="flex items-start gap-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg transition-colors duration-200">
                  <div className="size-10 rounded-lg bg-purple-100 dark:bg-purple-900/20 flex items-center justify-center">
                    <Scale className="size-5 text-purple-500 dark:text-purple-400" />
                  </div>
                  <div>
...
{user.height !==0 &&(
 <div className="flex items-start gap-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg transition-colors duration-200">
                  <div className="size-10 rounded-lg bg-green-100 dark:bg-green-900/20 flex items-center justify-center">
                    <User className="size-5 text-green-500 dark:text-green-400" />
                  </div>
                  <div>
...
{/* Goal */}
 <div className="flex items-start gap-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg transition-colors duration-200">
                  <div className="size-10 rounded-lg bg-orange-100 dark:bg-orange-900/20 flex items-center justify-center">
                    <Target className="size-5 text-orange-500 dark:text-orange-400" />
                  </div>
                  <div>
              ...
              <Button variant="secondary" onClick={() => setIsEditing(true)} className="w-full mt-4 text-green-500 hover:text-green-600 background-transparent border border-orange-500 hover:border-green-600">
                Edit Profile
              </Button>
            </>
          )}
...
<Card>
  ...
  <div className="grid grid-cols-2 gap-4  mt-4 hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors duration-200">
    <div className="text-center p-4 bg-emerald-50 dark:bg-emerlad-900/10 rounded-xl hover :bg-emerald-100 dark:hover:bg-emerald-900/20 transition-colors duration-200">
<p className="text-2xl font-bold text-slate-800 dark:text-emerald-400 hover:text-emerald-500 transition-colors duration-200">
  {stats.totalFoodEntries}
</p>
<p className="text-sm text-slate-500 dark:text-slate-400 hover:text-emerald-500 transition-colors duration-200">Food Entries</p>
    </div>
    
  <div  className="text-center p-4 bg-blue-50 dark:bg-blue-900/10 rounded-xl hover:bg-blue-100 dark:hover:bg-blue-900/20 transition-colors duration-200">
   <p className="text-2xl font-bold text-slate-800 dark:text-green-400 hover:text-green-500 transition-colors duration-200">
     {stats.totalActivityEntries}
   </p>
   <p className="text-sm text-green-500 dark:text-green-400 hover:text-green-600 transition-colors duration-200">
     Activities
   </p>
  </div>
...
<div className="lg:hidden">
  ...
  dark:hover:text-slate-200 rounded-lg transition-color 
